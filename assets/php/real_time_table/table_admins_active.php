...
<div class="table-responsive p-0">
    <table class="table align-items-center mb-0">
        ...
        <tbody>
        <?php
        $List_Admin = mysqli_query($conn,"SELECT * FROM `vw_employees` WHERE `Status` = 'active' AND `Type` = 'admin'");
        $student_row_count = 1;

        if ($List_Admin->num_rows>0){ 
            $view_btn = 'd-block';
        while ($row_data=$List_Admin-> fetch_assoc()){?>
        <tr>
            ...
            <div class="d-flex px-2 py-1">
                ...
                <p class="text-xs text-secondary mb-0"><?=$row_data['Employee ID']?></p>
                </div>
            </div>
            </td>
            <td>
            ...
            <td class="align-middle text-center">
            <span class="badge badge-sm bg-gradient-success"><?=$row_data['Status']?></span>
            </td>
        </tr>
    ...
    <div class="card-footer py-0 mb-3 <?=$view_btn?>">
        <a href="admin.php" class="btn btn-outline-secondary w-100 btn-sm mb-0"> View All </a>
    </div>
</div>
<?php
?>